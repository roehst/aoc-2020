...
data Op = Op { opName :: String, opArg :: Int } deriving Show
...
execute1 program cur acc visited =
    if cur `elem` visited then
        acc
    else
        if cur < length program then
            let visited' = S.insert cur visited in
            case program !! cur of
                Op "acc" arg -> execute1 program (cur + 1) (acc + arg) visited'
...
execute2 program cur acc visited =
    if cur `elem` visited then
        ...
        if cur < length program then
            let visited' = S.insert cur visited in
            case program !! cur of
                Op "acc" arg -> execute2 program (cur + 1) (acc + arg) visited'
